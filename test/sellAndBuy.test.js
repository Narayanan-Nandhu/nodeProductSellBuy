const SellBuy =require("../src/mongoose/models/sellAndbuy");
...
test.only("saving product", async () => {
...
test.only("saving product - invalid product name", async () => {
  expect(await getSellBuyProductLength()).toBe(4);
...
test.only("saving subjects - invalid cost price", async () => {
  expect(await getSellBuyProductLength()).toBe(4);
...
test.only("patch product - error", async () => {
  expect(await getSellBuyProductLength()).toBe(4);
...
test.only("getting all product data", async () => {
...
test.only("getting product data - Laptop", async () => {
...
test.only("getting product data - Mobile", async () => {
...
test.only("getting product data - lowerCostPrice", async () => {
...
test.only("getting product data - higherCostPrice", async () => {
...
test.only("getting product data - lowerSoldPrice", async () => {
  const response = await request(app).get("/sellProduct?sortBy=lowerSoldPrice");
  console.log('respoinse', response.body)
  expect(response.status).toBe(200);
  expect(response.body.length).toBe(4);
  expect(response.body[0].productName).toEqual("Table");
  expect(response.body[0].costPrice).toEqual(3000);
...
test.only("getting product data - higherSoldPrice", async () => {
...
test.only("deleting 2nd product", async () => {
  expect(await getSellBuyProductLength()).toBe(4);
...
test.only("deleting 2nd product-error", async () => {
  expect(await getSellBuyProductLength()).toBe(4);
